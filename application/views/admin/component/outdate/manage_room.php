<div class="d-flex  justify-content-center align-items-center gap-2 ">
            ...
            data-reserved-id="<?= $row['reserv_id']?>" class="btn btn-primary btn-action view-expire-button">
            <i class="bi bi-search"></i>
            </button>

            <button data-r-id="<?= $row['r_id']?>" data-s-id="<?= $row['r_s_id'] ?>" data-r-table="<?= ($row['r_s_id'] == 1) ? 'music' : (($row['r_s_id'] == 2) ? 'vdo' : 'mini') ?>"
            data-reserved-id="<?= $row['reserv_id']?>" data-st-id="<?= $row['st_id'] ?>" class="btn btn-success btn-action activation-button">
