...
<script>
    ...
    function showDeleteExpireConfirmation(id, name, s_id) {
     ...
     $(document).on('click', '.delete-expire-button', function () {
        const get_id = $(this).data('reserv-id');
        const name = $(this).data('user-fullname');
        const service_id = $(this).data('r-s-id');
...
<script>
function showActivationRoom(id, st_id, s_id,table,) {
        Swal.fire({
            title: 'คุณแน่ใจหรือไม่?',
            text: 'คุณแน่ใจใช่ใหมว่าจะเปิดใช้งานห้องของ ' + st_id + ' ใหม่อีกครั้ง ?',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = '<?= base_url("index.php/admin/reserv/activate/") ?>' + id;

                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = 'reserv_id';
                input.value = id;
    ...
    $(document).on('click', '.activation-button', function () {
        const reserv_id = $(this).data('reserved-id');
            const r_id = $(this).data('r-id');
            const table = $(this).data('r-table');
            const st_id = $(this).data('st-id');
            const service_id = $(this).data('check');
            console.log(table)
        showActivationRoom(reserv_id, st_id, service_id,table);
    });
