...
<div class="container">
    <div class="">
        <table class="table table-striped nowrap" style="width:100%" id="Table">
            ...
            if ($table === "time") {
                echo $this->load->view('admin/time_system_setting/time_table', [
                    'rows' => $rows,
                    'url' => 'music'
                ], true);
            }
...
<div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <form method="post" id="modal-form" action="<?= base_url() ?>index.php/admin/system/time/setting/submit"
                ...
                <div class="modal-body  d-flex justify-content-center">
                    <div class="col-10 col-sm-10 pb-4 col-md-10 col-lg-10 text-start">
                        ...
                        <div class="d-flex gap-4">
                            <div class="col-lg-6">
                                <label for="st_id">เลือกเวลาเปิดระบบ</label>
                            ...
                            <div class="col-lg-6">
                                <label for="st_id">เลือกเวลาปิดระบบ</label>
                                <div class="input-group">
...
<script>
    $(document).ready(function () {
        $('.edit-time-button').click(function () {
            var time_id = $(this).data('time-id'); // Get time_id from button
            var start = $(this).data('t-start'); // Get start from button
            var end = $(this).data('t-end'); // Get end from button
            $('#time_id').val(time_id); // Set time_id in the hidden input
            $('#timepicker_start').val(start); // Set start in the hidden input
            $('#timepicker_end').val(end); // Set end in the hidden input
    ...
    function update_time(event) {
        ...
        if (!time_start) {
            showSweet('warn', 'โปรดใส่เวลาเปิดระบบ')
            return false; // Stop execution if input is empty
        } else if (!time_end) {
            showSweet('warn', 'โปรดใส่เวลาปิดระบบ')
            return false; // Stop execution if input is empty
        } 
        Toast.fire({
            icon: "success",
